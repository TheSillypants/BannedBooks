...
p2_3 <- read.csv("https://raw.githubusercontent.com/TheSillypants/BannedBooks/refs/heads/main/PEN%20America's%20Index%20of%20School%20Book%20Bans%20(July%201%2C%202022%20-%20June%2030%2C%202023)%20-%20Sorted%20by%20State%20%26%20District.csv")

p3_4 <- read.csv("https://raw.githubusercontent.com/TheSillypants/BannedBooks/refs/heads/main/PEN%20America's%20Index%20of%20School%20Book%20Bans%20(July%201%2C%202023%20-%20June%2030%2C%202024)%20-%20Sorted%20by%20State%20%26%20District.csv")
```
...
p2_3 <- read.csv("https://raw.githubusercontent.com/TheSillypants/BannedBooks/refs/heads/main/PEN%20America's%20Index%20of%20School%20Book%20Bans%20(July%201%2C%202022%20-%20June%2030%2C%202023)%20-%20Sorted%20by%20State%20%26%20District.csv", skip =2)
p2_3 <- p2_3 %>% mutate(Year = "2022")
p2_3 <- p2_3 %>% mutate(E.Year = "2023")
# When I (Amber) was combining all of our code, I ran into the issue where 22-23 had data in the column names rather than column names, which did not allow for the data to be combined. This is my fix for that

colnames(p2_3) <- c("Title", "Author", "Secondary.Author.s", "Illustrator.s", "Translator.s", "Series.Name", "State", "District", "Date.of.Challenge.Removal", "Ban.Status", "Origin.of.Challenge")

p3_4 <- read.csv("https://raw.githubusercontent.com/TheSillypants/BannedBooks/refs/heads/main/PEN%20America's%20Index%20of%20School%20Book%20Bans%20(July%201%2C%202023%20-%20June%2030%2C%202024)%20-%20Sorted%20by%20State%20%26%20District.csv", skip=2)
p3_4 <- p3_4 %>% rename("Origin.of.Challenge" = "Initiating.Action")
p3_4 <- p3_4 %>% mutate(Year = "2023")
p3_4 <- p3_4 %>% mutate(E.Year = "2024")

#I ran into issues again when getting the data all on one document, as p1_2 and p2_3 said they would not merge as the column names did not match. This was my solution

colnames(p1_2) <- colnames(p2_3)

pen <- rbind(p2_3, p1_2)

#I did the same for p3_4 to the now merged pen.

colnames(p3_4) <- colnames(pen)
...
The new plan consisted of removing all dupes from the descriptions by writing a function that would append the descriptions to the array. A similar function would then be written to do the same write for the political data.
...
kds2 <- kds2[c("Title", "Description","Genre")]

test <- left_join(pen, kds2, by = c("Title"))
print(test)
```

After successfully joining the descriptions and genres to the Pen data, it then came time to do the same with the political data, so all the data that everyone would use for the later sections of the project would all be found in one location.
...
One of the first things we wanted to look at was what was banned more often than not. This consists of what genres were more banned than other, which words weremost frequently in the descriptions of banned books, along with what the most banned book out of this data was.
...
# The only reason why I (Amber) am not calling the final version of the dataset from the code above for all of our code is because of the difficulty I had in getting the cleaned data finish all the steps on my computer when putting them together.
```
...
for (word in genrekeywords) {
  ...
  data[[colname]] <- ifelse(str_detect(cleaned_data$genres, regex(pattern, ignore_case = TRUE)), 1, 0)
